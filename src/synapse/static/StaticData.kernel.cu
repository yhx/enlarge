...
__global__ void update_static_3(Connection *connection, StaticData *data, real *buffer, const uinteger_t *firedTable, const uinteger_t *firedTableSizes, size_t firedTableCap, size_t num, size_t start_id, int time)
{
	int delayLength = connection->maxDelay - connection->minDelay + 1;
	
	int d_para = gridDim.y;
	int d_offset = delayLength % d_para;
	int d_avg_up = (delayLength + d_para - 1) / d_para;
	// int d_start = 0;
	// int d_end = 0;
//  int d_idx = blockIdx.y;
//  if (d_idx < d_offset) {
//   d_start = d_idx * (d_avg_up);
//   d_end = d_start + d_avg_up;
//  } else if (d_idx < delayLength) { 
//   d_start = d_idx * (d_avg_up - 1) + d_offset;
//   d_end = d_start + d_avg_up - 1;
//  } else {
//   d_start = 0;
//   d_end = 0;
//  }

	uinteger_t mean_ft_size = 0;
	for(int i = 0; i < delayLength; i++)
	{
		mean_ft_size += firedTableSizes[(time+delayLength-i)%(connection->maxDelay+1)];
	}

	mean_ft_size /= gridDim.y;
	...
	for(int i = 0; i < delayLength; i++)
	{
		temp += firedTableSizes[(time+delayLength-i)%(connection->maxDelay+1)];
		if(temp > mean_ft_size)
		{
			idx += 1;
			if(idx == blockIdx.y)
			{
...
void cudaUpdateStatic(Connection * connection, void *data, real *buffer, uinteger_t *firedTable, uinteger_t *firedTableSizes, size_t firedTableCap, size_t num, size_t start_id, int time, BlockSize *pSize, cudaStream_t cuda_stream)
{
	...
	// case 1:
	update_dense_static_hit<<<pSize->gridSize, pSize->blockSize, 0, cuda_stream>>>((Connection *)connection,  (StaticData *)data, buffer, firedTable, firedTableSizes, firedTableCap, num, start_id, time);
