...
int main(int argc, char **argv)
{
    ...
    if (node_id == 0) {
        ...
        for (int i = 0; i < N * N; ++i) {
            weight1_2[i] = weight1_2[i] * ((i % 30 + 30.) / 15. - 15.);
            weight2_3[i] = weight2_3[i] * ((i % 30 + 30.) / 15. - 15.);
            weight3_4[i] = weight3_4[i] * ((i % 30 + 39.) / 15. - 15.);
            weight4_5[i] = weight4_5[i] * ((i % 30 + 30.) / 15. - 15.);
            weight1_4[i] = weight1_4[i] * ((i % 30 + 30.) / 15. - 15.);
        }
        ...
        real *poisson_means = getConstArray((real)(2957.5581111017573 * dt), N);
        real *poisson_weights = getConstArray(219.52123382302113, N);
        real *poisson_delay = getConstArray(dt, N); 
        for (int i = 1; i <= depth; ++i) {
            c.connect_poisson_generator(g[i], poisson_means, poisson_weights, poisson_delay, NULL);
        }
