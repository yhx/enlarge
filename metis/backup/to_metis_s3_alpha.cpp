...
int to_metis(const char *in_name, const char *out_name, idx_t nparts, const char *w_name, int alpha, int beta)
{
	idx_t *vwgt = NULL;
	size_t n_t = 0;
...
{
	if (argc == 7) {
		printf("Usuage: to_metis input_file output_file parts weight_file alpha beta\n");
		exit(-1);
	}
