...
int to_metis(const char *graph_name, const char *part_name, const int nparts)
{
	...
	for (size_t n=0; n<neuron_num; n++) {
		size_t size = 0;
		fread_c(&size, 1, graph_org);
		if (size > 0) {
			tmp.resize(size);
			fread_c(tmp.data(), size, graph);
			for (size_t v=0; v<size; v++) {
				vec_conn[n].insert(tmp[v]);
			}
		}
	}
	tmp.clear();
	...
	for (int p=0; p<nparts; p++) {
		...
		FILE *f = fopen_c((name+"_"+std::to_string(p)).c_str(), "wb+");
		...
		for (auto elem : part_org[p]) {
			subgraph[count].resize(min(t_s, graph_org[elem].size()));
			auto iter = set_intersection(part_org[i].begin(), part_org[i].end, graph_org[elem].begin(), graph_org[elem].end(), subgraph[count].begin());
		...
		for (size_t i=0; i<t_s; i++) {
			...
			fwrite_c(&tmp, 1, f);
			fwrite_c(subbraph[i].data(), tmp, f);
		}
		fclose(f);
...
{
	if (argc != 4) {
		printf("Usuage: to_metis origin_graph part_res part_num\n");
		exit(-1);
	}
