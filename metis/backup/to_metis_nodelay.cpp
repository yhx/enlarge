	...
	vector<vector<size_t>> vec_conn;
	{
		vector<unordered_set<size_t>> ud_conn;
		{
			...
			for (size_t n=0; n<neuron_num; n++) {
				...
				conn[n].clear();
				map<unsigned int, vector<size_t>>().swap(conn[n]);
			}
			conn.clear();
		...
		for (size_t n=0; n<neuron_num; n++) {
			for (auto i=ud_conn[n].begin(); i!=ud_conn[n].end(); i++) {
				vec_conn[n].push_back(*i);
				vec_conn[*i].push_back(n);
			}
		}
		...
		for (size_t n=0; n<neuron_num; n++) {
			ud_conn[n].clear();
			unordered_set<size_t>().swap(ud_conn[n]);
		}
		ud_conn.clear();
		vector<unordered_set<size_t>>().swap(ud_conn);
	}
	printf("Clear vec %lf\n", getCurrentRSS()/1024.0/1024.0/1024.0);
